...
/**
 * Quarter view — two sections:
 *  1. Multi-series line chart spanning ~65 trading days, all indices
 *     normalised to % return from the first day.
 *  2. Summary table: quarter-start price, latest price, total return.
 */
export function QuarterView() {
  ...
  const summaryRows = INDICES.map((index, i) => {
    ...
    return { index, start, end, ret, startDate, endDate, color: INDEX_COLORS[i] };
  ...
  // Derive the shared date window for the subtitle (from first index with data)
  const windowStart = summaryRows[0]?.startDate ?? '';
  const windowEnd   = summaryRows[0]?.endDate   ?? '';
    ...
    <div>
      ...
      <section className={styles.section}>
        ...
        <p className={styles.subtitle}>
          Normalised to the first trading day of the quarter — shows relative
      ...
      {!loading && summaryRows.length > 0 && (
        <section className={styles.section}>
          <h2 className={styles.sectionTitle}>Quarter Summary</h2>
